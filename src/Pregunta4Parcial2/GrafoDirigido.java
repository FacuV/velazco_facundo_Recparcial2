...
public class GrafoDirigido<T> extends Grafo<T> {
    ...
    //Pregunta 4
    public List<Vertice<T>> camino(Vertice<T> inicio,Vertice<T> fin){
        return camino(inicio,fin,new ArrayList<>());
    }
    private List<Vertice<T>> camino(Vertice<T> inicio, Vertice<T> fin, List<Vertice<T>> resultado){
        resultado.add(inicio);
        if(inicio.equals(fin)){return resultado;}
        for(Vertice<T> ady: this.getAdyacentes(inicio)){
            if(this.gradoSalida(ady) >= 2 || ady.equals(fin)){
                    return camino(ady,fin,resultado);
            }
        }
        return null;
    }
